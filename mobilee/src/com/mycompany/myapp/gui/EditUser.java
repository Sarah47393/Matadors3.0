...
public class EditUser extends Form {
 private String fileNameInServer = "";
     public  EditUser(Form previous, User ev,int ez) {
 ...
 int Password5 = Float.parseFloat(ev.getPassword().toString());*/
      TextField tfNom = new TextField(s1);
        ...
        btnValider.addActionListener(( e)-> {
            {  
                try {
                if ((tfCin.getText().length()==0)||(tfNom.getText().length()==0)||(tfPrenom.getText().length()==0)||(tfPassword.getText().length()==0)||(tfDatenaissance.getText().length()==0)||(tfRole.getText().length()==0)||(tfAccess.getText().length()==0))
